...
import {
  PRODUCTS, getProductsByCategory, sortProducts, productThumb,
  CATEGORY_COLORS,
...
function catClass(cat: ProductCategory) {
  return `market-cat-${cat.toLowerCase()}`
}
...
function ProductCard({ product, onPreview }: CardProps) {
  ...
  const cardRef  = useRef<HTMLDivElement>(null)   // tilt target
  ...
  const handleAddToCart = useCallback(() => {
    ...
    // Small thumbnail clone launches toward the cart icon
  ...
  return (
    <div
      ref={wrapRef}
      className={`market-card-wrap ${catClass(product.category)}`}
    ...
    >
      <div ref={cardRef} className="market-card">
        ...
        <div className="px-4 pt-3 pb-4 flex flex-col gap-2">
          ...
          {/* Subtitle */}
          <p className="font-mono text-ink-ash/50 text-[0.56rem] tracking-wide leading-none">
            {product.subtitle}
          </p>
          ...
          <div className="flex items-end gap-2 mt-1">
            ...
            <span className="font-mono text-ink-ash/40 text-[0.58rem] mb-0.5">
              {fmtUSD(product.priceUSD)}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <button
              ...
              className="market-add-btn ink-flood-up"
...
// ─── Filter Bar with sliding underline ───────────────────────────────────────
...
export default function MarketplacePage() {
  ...
  const visible = useMemo(
    () => sortProducts(getProductsByCategory(activeCat), sortBy),
    [activeCat, sortBy],
  )

  // ── Hero pill stagger (on mount) ─────────────────────────────────────────
  useEffect(() => {
  ...
  // ── Re-animate on filter / sort change ───────────────────────────────────
  useEffect(() => {
    ...
    <main className="min-h-screen bg-ink-void">
      ...
      <section
        ...
        className="relative flex flex-col items-center justify-center text-center
          min-h-[55vh] pt-16 pb-12 px-6 overflow-hidden"
      >
        {/* Background image */}
        <img src="/Images/bg (2).jpg" alt="" aria-hidden="true"
          ...
          <div ref={pillsRef} className="flex flex-wrap items-center justify-center gap-3 mt-2">
            {HERO_PILLS.map(({ cat, dot }) => (
              <div key={cat} className="market-hero-pill-wrap flex items-center gap-3">
                ...
                {dot && (
                  <span className="font-mono text-ink-ash/30 text-xs select-none">{dot}</span>
                )}
              </div>
            ))}
          </div>
      ...
      <section className="max-w-[1400px] mx-auto px-5 md:px-10 py-12 md:py-16">
        ...
        {visible.length === 0 && (
          <div className="py-24 flex flex-col items-center gap-4">
            <Thunder as="p" size="card" weight={400} className="text-ink-paper/15 leading-none">
              NOTHING HERE YET
            </Thunder>
          </div>
        )}
      ...
      {/* ═══ QUICK PREVIEW MODAL (lightweight) ══════════════════════════════ */}
      {previewProduct && (
        <div
        ...
        >
          ...
          >
            <img
              src={productThumb(previewProduct.imageId)}
            ...
            <div className="p-5">
              ...
              <Thunder as="h2" size="card" weight={400} className="text-ink-paper leading-tight mb-2">
                {previewProduct.title}
              </Thunder>
              <Body size="sm" className="text-ink-ash/70 leading-relaxed mb-4">
