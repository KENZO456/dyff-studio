...
function SeriesTile({ series }: { series: typeof AUDIO_SERIES[0] }) {
  return (
    <Link
    ...
    >
      ...
      <div className="series-cover relative overflow-hidden rounded-sm aspect-[3/4] mb-3">
        ...
        {/* Soundwave bars inside cover (accent color from CSS var) */}
        <div className="absolute bottom-6 left-0 right-0 z-[2] px-4 flex items-end justify-center gap-[2px] h-12">
          {TILE_BARS.map((bar, i) => (
            <div
              ...
              style={
                { '--bar-h': `${bar.h}%`, '--bar-delay': `${bar.delay + i * 0.04}s` } as React.CSSProperties
              }
            />
          ))}
        </div>

        {/* Gradient overlay — pulls from --series-accent-dim */}
        ...
        <div
          ...
          aria-hidden="true"
        >
          <span className="tile-watermark font-thunder uppercase text-center leading-none px-3">
            {series.title}
          </span>
        </div>
        ...
        {/* Hover reveal border — picks up --series-accent from parent class */}
        <div className="tile-hover-border absolute inset-0 z-[5] rounded-sm pointer-events-none" />
      </div>

      {/* Below-card subtitle */}
      <p className="font-mono text-ink-ash/60 text-[0.58rem] leading-snug px-1 line-clamp-2">
        {series.subtitle}
      </p>
    </Link>
  )
    ...
    <main className="min-h-screen bg-ink-void">
      ...
      <section className="relative min-h-[80vh] flex flex-col items-center justify-center overflow-hidden px-6 py-32">
        ...
        <div className="absolute inset-0 z-[3] flex items-center justify-center select-none pointer-events-none" aria-hidden="true">
          <span className="audio-hero-ghost font-thunder uppercase text-ink-paper/[0.03] leading-none">
            AUDIO
          </span>
        </div>
        ...
        <div className="relative z-[4] flex flex-col items-center text-center gap-8 max-w-2xl">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="font-mono text-ink-ash/60 text-[0.6rem] tracking-[0.25em] uppercase">
              {AUDIO_SERIES.length} Active Series
            </span>
            <div className="w-12 h-px bg-ink-green" />
          </div>
        </div>
      ...
      <section className="relative py-16 md:py-24 border-t border-ink-ash/10">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          ...
          <div className="flex items-end justify-between mb-10">
            <div>
              <Label variant="tag" className="text-ink-ash/60 mb-3">
                CURRENT CATALOGUE
              </Label>
          ...
          <div ref={gridRef} className="audio-shelf">
            {AUDIO_SERIES.map(series => (
              <SeriesTile key={series.slug} series={series} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-ink-ash/10 py-16 px-6 md:px-10">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <Thunder
            ...
            size="display"
            weight={400}
            className="text-ink-paper/20 leading-none select-none"
            aria-hidden="true"
          >
            THE INK<br />SPEAKS.
          </Thunder>
